...
describe('Testing Authorization routes', () => {
  ...
  it('Users with role \'user\' should only be able to read', async () => {
    ...
    //const readFood = await request.get('routes/v1/food').set('Authorization', `Bearer ${newUser.body.token}`);
  ...
  it('Should deny user that can\'t write', async () => {
    ...
    //const writeFood = await request.post('routes/v1/food').set('Authorization', `Bearer ${user.token}`);
